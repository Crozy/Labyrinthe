...
public class FrameView extends Pane{
	...
	static int sizeY;
	static boolean init;
	static Stage stageScore;

	private Stage stage;

	private int cpt = 0; //compteur de pièces
	...
	public void paint() {
		...
		int xp =(sizeX-1)*(CELL*SPAN+WALL*SPAN);
		int yp =(sizeY-1)*(CELL*SPAN+WALL*SPAN);
		...
		for(int i=0; i<Controller.getNbMonster()+1; i++) {
			...
			xp =Controller.positionPersonnage(i).GetX()*(CELL*SPAN+WALL*SPAN);
			yp =Controller.positionPersonnage(i).GetY()*(CELL*SPAN+WALL*SPAN);
			square = new Rectangle(xp+2*SPAN,yp+3*SPAN,CELL*SPAN,CELL*SPAN);
			imagePattern = new ImagePattern(image);
			square.setFill(imagePattern);
			getChildren().add(square);
	...
	public void mettreAJourScore(){
		...
		text1 = new Text("Niveau ");
		text1.setFont(Font.font("Verdana", FontWeight.BOLD, 30));
		text1.setFill(Color.RED);
		text2 = new Text(": ");
		text2.setFont(Font.font("Verdana", FontWeight.BOLD, 30));
		text2.setFill(Color.ORANGE);
		text3 = new Text("" + Controller.getNiveau());
		text3.setFont(Font.font("Verdana", FontWeight.BOLD, 30));
		text3.setFill(Color.GREEN);
		textFlow.getChildren().addAll(text1, text2, text3);
		paneScore.getChildren().add(textFlow);
