...
public class Graph implements Comparable<Graph> {
	...
	public void rendreJouable() {
		for (int i = 0; i < h; i++) {
			...
			voisin[x + y * w].changerMatiereArrete(direction, Arrete.Matiere.corridor);
		}
	}
	...
	// Initialise le graph en creant tout les sommets et en les connectant entre eux
	// via des arrete tous initialiser a mur
	...
	public Arrete GetPorte(CarteDistance carte) {
		Arrete tmp;
		Point p;
		boolean seulissu1 = false;
		boolean seulissu2 = false;
		do {
			do {
				tmp = voisin[(int) (Math.random() * w) + (int) (Math.random() * h) * w]
						.GetArrete((int) (Math.random() * 4));
			} while (tmp.getType() != Arrete.Matiere.corridor);
			if (tmp.getPoint1() == null) {
				p = tmp.getPoint2();
				seulissu1 = true;
			} else {
				p = tmp.getPoint1();
				seulissu1 = carte.seulIssu(tmp.getPoint1());
			}
			if (tmp.getPoint2() == null) {
				seulissu2 = true;
			} else {
				seulissu2 = carte.seulIssu(tmp.getPoint2());
			}
		} while (!seulissu1 && !seulissu2 && carte.tab[p.GetX()][p.GetY()] <= Model.getLabyrinth().h);
		return tmp;
	}
}
